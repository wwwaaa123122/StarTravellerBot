...
from .api import API
from .auth import API_BASE_PROD, WSS_BASE_PROD, AccessTokenManager
...
class Client:
    ...
    async def start(self, appid: str, secret: str) -> None:
        self._appid = appid
        self._secret = secret
        self._token_manager = AccessTokenManager(appid, secret, base_url=API_BASE_PROD)
        self.api = API(HTTPClient(API_BASE_PROD, self._token_manager))
        self._state = ConnectionState(self)
        self._gateway = GatewayClient(WSS_BASE_PROD, self._token_manager, self.intents, self._state)

        _log.info("启动 QQ 开放平台客户端")
