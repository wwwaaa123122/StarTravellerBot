...
# 本地模块（无父项目依赖）
from core import BotContext, VERSION_NAME
...
class XCLRClient(botpy.Client):
    ...
    async def on_group_message_create(self, message: Any):
        ...
        try:
            ...
            # 尝试匹配插件（全量消息排除签到、好感度，避免误触发）
            plugin_result = await self._try_plugins(message, order, skip_plugins={"checkin", "affection"})
            if plugin_result:
                return
    ...
    async def _handle_ai_chat_c2c(self, message: Any, order: str):
        ...
        try:
            ...
            if ROLEPLAY_ENABLED:
                ...
                sys_prompt = f"你是{self.bot_name}，一个友善的 AI 助手。和你说话的人叫用户，用中文简洁回复。"
    ...
    async def _handle_ai_chat_dms(self, message: DirectMessage, order: str):
        ...
        try:
            ...
            if ROLEPLAY_ENABLED:
                ...
                sys_prompt = f"你是{self.bot_name}，一个友善的 AI 助手。和你说话的人叫{user_name}，用中文简洁回复。"
