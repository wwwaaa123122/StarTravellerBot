...
前端为纯静态实现（`webadmin/static/`，原生 HTML/CSS/JS，无构建依赖），登录后进入管理面板：

| 页面 | 功能 |
| :--- | :--- |
| 总览 | 统计卡片（用户/消息/AI 调用/Token/插件/积分/RAG/访问）、近 14 天签到趋势、AI 调用统计（请求数/Token/延迟/错误率/费用）、角色分布 |
| 插件 | 插件列表（名称/关键字/帮助/状态），开关切换即写入 reload.flag，机器人 ~10s 内热重载生效 |
| 权限 | ROOT_User（管理员）、black_list（黑名单，消息被静默忽略）、allow_ai 开关 |
| AI 设置 | 提供商（DeepSeek/OpenAI 兼容 / Gemini）、模型、Base URL、Max Tokens、Temperature、API Key（掩码显示，留空不修改） |
| 用户 | 全部签到用户（昵称 / OpenID / 积分 / 好感度 / 连续天数 / 最近签到 / 角色） |
| 记忆 | 检索所有用户的 RAG 对话记忆记录 |
| 定时 | 定时群发配置（启用状态 / 发送时间 / 通知群数 / 最近发送日期 / 内容） |
| 配置 | 机器人配置（敏感字段脱敏展示）与运行信息 |

机器人状态（在线/离线）每 15s 自动刷新；登录采用 Bearer token（HMAC 签名，7 天有效），登录接口有 IP 级频率限制。
