...
- **插件系统**: 动态加载，关键字触发 + 全局匹配，webadmin 开关热重载（~10s 生效）
- **AI 对话**: Provider 抽象（DeepSeek / Gemini / 任意 OpenAI 兼容服务），支持角色扮演与 Function Calling，调用费用统计
- **内置插件**: 签到、天气、Ping、一言、随机图、MC 状态、域名查询、QR 码生成、踢人、定时发送等
- **Web 管理后台**: 状态监控、插件开关/热重载、权限（ROOT_User/黑名单）、AI 设置、统计
- **可靠性**: 消息去重（防网关重放）、黑名单静默过滤、调度器 tzdata 兜底
